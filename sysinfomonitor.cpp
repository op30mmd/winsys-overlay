...
SysInfoMonitor::SysInfoMonitor(QObject *parent) : QObject(parent)
{
    ...
    if (FAILED(hres)) {
        qWarning() << "Failed to initialize COM library. Error code = 0x" << std::hex << hres;
    } else {
        ...
        if (FAILED(hres)) {
            qWarning() << "Failed to initialize security. Error code = 0x" << std::hex << hres;
            CoUninitialize();
        } else {
            ...
            if (FAILED(hres)) {
                qWarning() << "Failed to create IWbemLocator object. Error code = 0x" << std::hex << hres;
                CoUninitialize();
            } else {
                ...
                if (FAILED(hres)) {
                    qWarning() << "Could not connect to WMI namespace. Error code = 0x" << std::hex << hres;
                    m_pLocator->Release();
                    CoUninitialize();
                }
            }
...
void SysInfoMonitor::updateNetworkStats(SysInfo& info)
{
    ...
    ULONG bufferSize = 0;
    if (GetIfTable2(&bufferSize, nullptr) == ERROR_INSUFFICIENT_BUFFER) {
